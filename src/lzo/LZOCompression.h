...
public:
	...
	static std::vector<uint8_t> Compress(const std::vector<uint8_t>& data, bool* error = nullptr);
	...
	/// If an error occurs, error message will be sent to stdout and this function will return an empty vector, and <code>error</error> will be set to true
	static std::vector<uint8_t> Decompress(const std::vector<uint8_t> data, uint32_t decompressedSize, bool* error = nullptr);
};
